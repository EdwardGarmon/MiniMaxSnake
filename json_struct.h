...
#define JS_IF_CONSTEXPR(exp)                                                   \
  __pragma(warning(push)) __pragma(warning(disable : 4127)) if (exp)           \
...
struct DataRef {
   ...
   */
  template <size_t N>
  constexpr explicit DataRef(const char (&data)[N]) : data(data), size(N - 1) {}
...
static inline const unsigned char *lookup() {
  static const unsigned char tmp[] = {/*0*/ 4,
                                      0,
                                      0,
                                      0,
                                      0,
...
template <typename T> struct CallbackContainer;
} // namespace Internal

template <typename T> class RefCounter {
public:
  RefCounter() : callbackContainer(nullptr), index(0) {}
...
public:
  ...
  Callback(std::function<T> &callback) : ref(0), callback(callback) {}
...
public:
  ...
  template <typename... Ts> void invokeCallbacks(Ts &...args) {
...
template <typename T> inline void RefCounter<T>::inc() {
  if (callbackContainer)
    callbackContainer->inc(index);
}

template <typename T> inline void RefCounter<T>::dec() {
  if (callbackContainer)
...
public:
  ...
  NeedMoreDataCBRef
  registerNeedMoreDataCallback(std::function<void(Tokenizer &)> callback);
  ReleaseCBRef
  registerReleaseCallback(std::function<void(const char *)> &callback);
  ...
  Error updateErrorContext(Error error,
...
private:
  ...
  Error findStartOfNextValue(Type *type, const DataRef &json_data,
                             size_t *chars_ahead);
  ...
  Error populateFromDataRef(DataRef &data, Type &type,
                            const DataRef &json_data);
  static void populate_annonymous_token(const DataRef &data, Type type,
                                        Token &token);
  Error populateNextTokenFromDataRef(Token &next_token,
                                     const DataRef &json_data);
...
public:
  ...
  const BufferRequestCBRef
  addRequestBufferCallback(std::function<void(Serializer &)> callback);
...
inline Tokenizer::Tokenizer()
    : is_escaped(false), allow_ascii_properties(false), allow_new_lines(false),
      allow_superfluous_comma(false), expecting_prop_or_annonymous_data(false),
      continue_after_need_more_data(false), cursor_index(0),
      current_data_start(0), line_context(4), line_range_context(256),
      range_context(38), parsed_data_vector(nullptr) {
...
template <size_t N> inline void Tokenizer::addData(const char (&data)[N]) {
...
inline void Tokenizer::resetData(const std::vector<Token> *parsedData,
                                 size_t index) {
  for (auto &data_buffer : data_list)
    release_callbacks.invokeCallbacks(data_buffer.data);
  data_list.clear();
...
inline NeedMoreDataCBRef Tokenizer::registerNeedMoreDataCallback(
    std::function<void(Tokenizer &)> callback) {
  return need_more_data_callbacks.addCallback(callback);
}

inline ReleaseCBRef Tokenizer::registerReleaseCallback(
...
inline Error Tokenizer::nextToken(Token &next_token) {
  assert(!scope_counter.size() ||
         (scope_counter.back().type != JS::Type::ArrayEnd &&
          scope_counter.back().type != JS::Type::ObjectEnd));
  ...
  if (error == JS::Error::NoError) {
    if (next_token.value_type == Type::ArrayStart ||
        next_token.value_type == Type::ObjectStart)
      container_stack.push_back(next_token.value_type);
    if (next_token.value_type == Type::ArrayEnd) {
      assert(container_stack.size() &&
             container_stack.back() == JS::Type::ArrayStart);
      container_stack.pop_back();
    }
    if (next_token.value_type == Type::ObjectEnd) {
      assert(container_stack.size() &&
             container_stack.back() == JS::Type::ObjectStart);
      container_stack.pop_back();
    }
    if (scope_counter.size())
      scope_counter.back().handleType(next_token.value_type);
...
static bool
isValueInIntermediateToken(const Token &token,
                           const Internal::IntermediateToken &intermediate) {
...
inline void Tokenizer::copyFromValue(const Token &token,
...
inline void Tokenizer::copyIncludingValue(const Token &,
                                          std::string &to_buffer) {
  auto it =
      std::find_if(copy_buffers.begin(), copy_buffers.end(),
                   [&to_buffer](const std::pair<size_t, std::string *> &pair) {
                     return &to_buffer == pair.second;
                   });
  assert(it != copy_buffers.end());
  assert(it->first <= cursor_index);
  if (cursor_index - it->first != 0)
    to_buffer.append(data_list.front().data + it->first,
                     cursor_index - it->first);
...
inline std::string Tokenizer::makeErrorString() const {
  static_assert(sizeof(Internal::error_strings) /
                        sizeof *Internal::error_strings ==
  ...
  if (error_context.error < Error::UserDefinedErrors)
    retString +=
        std::string(" ") + Internal::error_strings[int(error_context.error)];
...
inline void Tokenizer::setErrorContextConfig(size_t lineContext,
                                             size_t rangeContext) {
...
inline Error Tokenizer::findStringEnd(const DataRef &json_data,
                                      size_t *chars_ahead) {
...
inline Error Tokenizer::findAsciiEnd(const DataRef &json_data,
                                     size_t *chars_ahead) {
  ...
  while (end < json_data.size) {
    while (end + 4 < json_data.size) {
      unsigned char lc = Internal::lookup()[(unsigned char)json_data.data[end]];
      if (!(lc & (Internal::AsciiLetters | Internal::Digits |
                  Internal::HatUnderscoreAprostoph)))
        break;
      lc = Internal::lookup()[(unsigned char)json_data.data[++end]];
      if (!(lc & (Internal::AsciiLetters | Internal::Digits |
                  Internal::HatUnderscoreAprostoph)))
...
inline Error Tokenizer::findNumberEnd(const DataRef &json_data,
                                      size_t *chars_ahead) {
  size_t end = cursor_index;
  while (end + 4 < json_data.size) {
    unsigned char lc = Internal::lookup()[(unsigned char)json_data.data[end]];
...
inline Error Tokenizer::findStartOfNextValue(Type *type,
  ...
  for (size_t current_pos = cursor_index; current_pos < json_data.size;
       current_pos++) {
...
inline Error Tokenizer::findDelimiter(const DataRef &json_data,
                                      size_t *chars_ahead) {
  ...
  for (size_t end = cursor_index; end < json_data.size; end++) {
    ...
    if (c == ':') {
      ...
      *chars_ahead = end - cursor_index;
      return Error::NoError;
    } else if (!(Internal::lookup()[(unsigned char)c] &
                 Internal::WhiteSpaceOrNull)) {
      return Error::ExpectedDelimiter;
    }
  }
  return Error::NeedMoreData;
...
inline Error Tokenizer::findTokenEnd(const DataRef &json_data,
                                     size_t *chars_ahead) {
  if (container_stack.empty())
    return Error::NoError;
  for (size_t end = cursor_index; end < json_data.size; end++) {
...
inline void Tokenizer::releaseFirstDataRef() {
  ...
  for (auto &copy_pair : copy_buffers) {
    std::string data(json_data.data + copy_pair.first,
                     json_data.size - copy_pair.first);
...
inline Error Tokenizer::populateFromDataRef(DataRef &data, Type &type,
                                            const DataRef &json_data) {
  size_t diff = 0;
  Error error = Error::NoError;
  data.size = 0;
...
inline void Tokenizer::populate_annonymous_token(const DataRef &data, Type type,
                                                 Token &token) {
  token.name = DataRef();
...
inline Error Tokenizer::populateNextTokenFromDataRef(Token &next_token,
                                                     const DataRef &json_data) {
  Token tmp_token;
  while (cursor_index < json_data.size) {
    ...
    case InTokenState::FindingName:
      ...
      if (error == Error::NeedMoreData) {
        if (property_state > InPropertyState::NoStartFound) {
          ...
          size_t to_null =
              Internal::strnlen(data.data, json_data.size - current_data_start);
      ...
      tmp_token.name_type =
          Internal::getType(type, tmp_token.name.data, tmp_token.name.size);
    ...
    case InTokenState::FindingDelimiter:
      ...
      if (error != Error::NoError) {
        if (intermediate_token.active == false) {
          intermediate_token.name.append(tmp_token.name.data,
                                         tmp_token.name.size);
      ...
      if (token_state == InTokenState::FindingName) {
        populate_annonymous_token(tmp_token.name, tmp_token.name_type,
                                  next_token);
        return Error::NoError;
      } else {
    ...
    case InTokenState::FindingData:
      ...
      tmp_token.value_type =
          Internal::getType(type, tmp_token.value.data, tmp_token.value.size);
...
inline Error Tokenizer::updateErrorContext(Error error,
  ...
  const DataRef json_data =
      parsed_data_vector && parsed_data_vector->size()
          ? DataRef(parsed_data_vector->front().value.data,
                    size_t(parsed_data_vector->back().value.data -
                           parsed_data_vector->front().value.data))
          : data_list.front();
  size_t real_cursor_index =
      parsed_data_vector && parsed_data_vector->size()
          ? size_t(parsed_data_vector->at(cursor_index).value.data -
                   json_data.data)
          : cursor_index;
  const size_t stop_back =
      real_cursor_index - std::min(real_cursor_index, line_range_context);
  const size_t stop_forward =
      std::min(real_cursor_index + line_range_context, json_data.size);
  ...
  for (cursor_back = real_cursor_index - 1; cursor_back > stop_back;
       cursor_back--) {
  ...
  for (cursor_forward = real_cursor_index; cursor_forward < stop_forward;
       cursor_forward++) {
    if (add_new_line) {
  ...
  if (lines.size() > 1) {
    ...
    for (auto &line : lines) {
      error_context.lines.push_back(
          std::string(json_data.data + line.start, line.end - line.start));
    }
    error_context.line = lines_back;
  } else {
    error_context.line = 0;

    size_t left = real_cursor_index > range_context
                      ? real_cursor_index - range_context
                      : 0;
    size_t right = real_cursor_index + range_context > json_data.size
                       ? json_data.size
                       : real_cursor_index + range_context;
    error_context.character = real_cursor_index - left;
    error_context.lines.push_back(
        std::string(json_data.data + left, right - left));
  }
  return error;
}
...
inline SerializerOptions::SerializerOptions(Style style)
    ...
    : m_shift_size(style == Compact ? 0 : 2), m_depth(0), m_style(style),
      m_convert_ascii_to_string(true), m_token_delimiter(","),
...
inline void SerializerOptions::setDepth(int depth) {
  ...
  m_prefix = m_style == Pretty ? std::string(depth * size_t(m_shift_size), ' ')
                               : std::string();
}
...
inline const BufferRequestCBRef Serializer::addRequestBufferCallback(
    std::function<void(Serializer &)> callback) {
...
static inline JS::Error
reformat(const char *data, size_t size, std::string &out,
  ...
  size_t last_pos = 0;
  auto cbref = serializer.addRequestBufferCallback(
      [&out, &last_pos](Serializer &serializer_p) {
        size_t end = out.size();
        out.resize(end * 2);
...
static inline JS::Error
reformat(const std::string &in, std::string &out,
         const SerializerOptions &options = SerializerOptions()) {
...
template <size_t size> struct GenSequence {
  using type =
      typename Merge<typename GenSequence<size / size_t(2)>::type,
...
template <size_t I, typename... Ts> struct TypeAt {
  ...
  using tuple_impl =
      Internal::TupleImpl<typename Internal::GenSequence<sizeof...(Ts)>::type,
                          Ts...>;
...
template <typename... Ts> struct Tuple {
  ...
  constexpr const typename TypeAt<Index, Ts...>::type &get() const {
    return static_cast<const typename TypeAt<Index, Ts...>::element &>(impl)
        .data;
  }
...
/*!  \private
 */
template <> struct Tuple<> { static constexpr const size_t size = 0; };

template <typename... Ts> constexpr Tuple<Ts...> makeTuple(Ts... args) {
...
struct JsonMeta {
  JsonMeta(size_t pos, bool is_array)
      : position(pos), size(1), skip(1), children(0), complex_children(0),
        is_array(is_array), has_data(false) {}
...
static inline std::vector<JsonMeta> metaForTokens(const JsonTokens &tokens) {
  ...
  for (size_t i = 0; i < tokens.data.size(); i++) {
    ...
    if (token.value_type == Type::ArrayEnd ||
      ...
      assert(parent.size());
      assert(meta[parent.back()].is_array ==
             (token.value_type == Type::ArrayEnd));
      parent.pop_back();
    } else {
      if (parent.size())
  ...
  assert(!parent.size()); // This assert may be triggered when JSON is invalid
                          // (e.g. when creating a DiffContext).
  return meta;
}

namespace Internal {
static inline size_t
findFirstChildWithData(const std::vector<JsonMeta> &meta_vec,
                       size_t start_index) {
...
template <typename T> struct IsOptionalType {
  ...
  static constexpr const bool value =
      sizeof(test_in_optional<T>(0)) == sizeof(yes);
};
...
struct ParseContext {
  ...
  template <size_t SIZE> explicit ParseContext(const char (&data)[SIZE]) {
  ...
  std::string makeErrorString() const {
    if (error == Error::MissingPropertyMember) {
      if (missing_members.size() == 0) {
        ...
        return std::string("JSON Object contained member not found in C++ "
                           "struct/class. JSON Object member is: ") +
      ...
      return std::string("JSON Object contained members not found in C++ "
                         "struct/class. JSON Object members are: ") +
             member_string;
      ...
      if (unassigned_required_members.size() == 0) {
        ...
        return std::string("C++ struct/class has a required member that is not "
                           "present in input JSON. The unassigned "
                           "C++ member is: ") +
      ...
      return std::string("C++ struct/class has required members that are not "
                         "present in the input JSON. The unassigned "
                         "C++ members are: ") +
             required_string;
    }
    if (tokenizer.errorContext().error == Error::NoError &&
        error != Error::NoError) {
      std::string retString("Error:");
 ...
 *
 * Create meta information where the primary name is argument name, and the
...
struct JsonStructBaseDummy {
  static_assert(
      sizeof(HasJsonStructBase<JS_OBJECT_T>::template test_in_base<JS_OBJECT_T>(
          nullptr)) == sizeof(typename HasJsonStructBase<JS_OBJECT_T>::yes),
      "Missing JS_OBJECT JS_OBJECT_EXTERNAL or TypeHandler specialisation\n");
  using TT = decltype(JS_OBJECT_T::template JsonStructBase<
                      JS_OBJECT_T>::js_static_meta_data_info());
  using ST = decltype(JS_OBJECT_T::template JsonStructBase<
                      JS_OBJECT_T>::js_static_meta_super_info());
  static inline constexpr const TT js_static_meta_data_info() {
    return JS_OBJECT_T::template JsonStructBase<
        JS_OBJECT_T>::js_static_meta_data_info();
  }
...
#define JS_INTERNAL_FIRST(...)                                                 \
  JS_INTERNAL_EXPAND(JS_INTERNAL_FIRST_(__VA_ARGS__))
#define JS_INTERNAL_SECOND(...)                                                \
  JS_INTERNAL_EXPAND(JS_INTERNAL_SECOND_(__VA_ARGS__))
#define JS_INTERNAL_EMPTY()
#define JS_INTERNAL_EVAL(...) JS_INTERNAL_EVAL1024(__VA_ARGS__)
#define JS_INTERNAL_EVAL1024(...)                                              \
  JS_INTERNAL_EVAL512(JS_INTERNAL_EVAL512(__VA_ARGS__))
#define JS_INTERNAL_EVAL512(...)                                               \
  JS_INTERNAL_EVAL256(JS_INTERNAL_EVAL256(__VA_ARGS__))
#define JS_INTERNAL_EVAL256(...)                                               \
  JS_INTERNAL_EVAL128(JS_INTERNAL_EVAL128(__VA_ARGS__))
#define JS_INTERNAL_EVAL128(...)                                               \
  JS_INTERNAL_EVAL64(JS_INTERNAL_EVAL64(__VA_ARGS__))
#define JS_INTERNAL_EVAL64(...)                                                \
  JS_INTERNAL_EVAL32(JS_INTERNAL_EVAL32(__VA_ARGS__))
#define JS_INTERNAL_EVAL32(...)                                                \
  JS_INTERNAL_EVAL16(JS_INTERNAL_EVAL16(__VA_ARGS__))
#define JS_INTERNAL_EVAL16(...)                                                \
  JS_INTERNAL_EVAL8(JS_INTERNAL_EVAL8(__VA_ARGS__))
...
#define JS_INTERNAL_NOT(x)                                                     \
  JS_INTERNAL_IS_PROBE(JS_INTERNAL_CAT(JS_INTERNAL__NOT_, x))
#define JS_INTERNAL__NOT_0 JS_INTERNAL_PROBE()

#define JS_INTERNAL_BOOL(x) JS_INTERNAL_NOT(JS_INTERNAL_NOT(x))

#define JS_INTERNAL_IF_ELSE(condition)                                         \
  JS_INTERNAL__IF_ELSE(JS_INTERNAL_BOOL(condition))
#define JS_INTERNAL__IF_ELSE(condition)                                        \
  JS_INTERNAL_CAT(JS_INTERNAL__IF_, condition)
...
#define JS_INTERNAL_HAS_MORE_THAN_ONE_ARGS(...)                                \
  JS_INTERNAL_BOOL(                                                            \
      JS_INTERNAL_SECOND(JS_INTERNAL__END_OF_ARGUMENTS_ __VA_ARGS__, 0, 0)())
...
#define JS_MEMBER_ALIASES(member, ...)                                         \
  JS_INTERNAL_EXPAND(                                                          \
      JS::makeMemberInfo(#member, &JS_OBJECT_T::member, __VA_ARGS__))
#define JS_MEMBER_WITH_NAME(member, name)                                      \
  JS::makeMemberInfo(name, &JS_OBJECT_T::member)
#define JS_MEMBER_WITH_NAME_AND_ALIASES(member, name, ...)                     \
  JS::makeMemberInfo(name, &JS_OBJECT_T::member, __VA_ARGS__)
...
#define JS_INTERNAL_MAKE_MEMBERS(...)                                          \
  ...
  (JS_INTERNAL_EXPAND(JS_INTERNAL_EVAL(                                        \
      JS_INTERNAL_MAP_MEMBER(JS::makeMemberInfo, __VA_ARGS__))))(              \
...
#define JS_INTERNAL_MAKE_SUPER_CLASSES(...)                                    \
  JS_INTERNAL_IF_ELSE(JS_INTERNAL_HAS_MORE_THAN_ONE_ARGS(__VA_ARGS__))         \
  (JS_INTERNAL_EXPAND(JS_INTERNAL_EVAL(                                        \
      JS_INTERNAL_MAP_SUPER(JS::makeSuperInfo, __VA_ARGS__))))(                \
      JS_INTERNAL_MAP_APPLY_SUPER(JS::makeSuperInfo, __VA_ARGS__))

#define JS_SUPER(...)                                                          \
  JS::makeTuple(JS_INTERNAL_EXPAND(JS_INTERNAL_MAKE_SUPER_CLASSES(__VA_ARGS__)))
...
#define JS_OBJECT_INTERNAL_IMPL(super_list, member_list)                       \
  template <typename JS_OBJECT_T> struct JsonStructBase {                      \
    ...
    static inline constexpr const decltype(super_list)                         \
    js_static_meta_super_info() {                                              \
      return super_list;                                                       \
    }                                                                          \
  }
...
#define JS_OBJECT(...)                                                         \
  JS_OBJECT_INTERNAL_IMPL(JS::makeTuple(), JS::makeTuple(__VA_ARGS__))
#define JS_OBJECT_WITH_SUPER(super_list, ...)                                  \
  JS_OBJECT_INTERNAL_IMPL(super_list, JS::makeTuple(__VA_ARGS__))

#define JS_OBJECT_EXTERNAL(Type, ...)                                          \
  JS_OBJECT_EXTERNAL_INTERNAL_IMPL(Type, JS::makeTuple(),                      \
                                   JS::makeTuple(__VA_ARGS__))
...
#define JS_OBJ(...)                                                            \
  JS_OBJECT_INTERNAL_IMPL(                                                     \
      JS::makeTuple(), JS::makeTuple(JS_INTERNAL_MAKE_MEMBERS(__VA_ARGS__)))
#define JS_OBJ_SUPER(super_list, ...)                                          \
  JS_OBJECT_INTERNAL_IMPL(                                                     \
      super_list, JS::makeTuple(JS_INTERNAL_MAKE_MEMBERS(__VA_ARGS__)))

#define JS_OBJ_EXT(Type, ...)                                                  \
  JS_OBJECT_EXTERNAL_INTERNAL_IMPL(                                            \
      Type, JS::makeTuple(),                                                   \
      JS::makeTuple(JS_INTERNAL_MAKE_MEMBERS(__VA_ARGS__)))
#define JS_OBJ_EXT_SUPER(Type, super_list, ...)                                \
  JS_OBJECT_EXTERNAL_INTERNAL_IMPL(                                            \
      Type, super_list, JS::makeTuple(JS_INTERNAL_MAKE_MEMBERS(__VA_ARGS__)))
...
constexpr StringLiteral<SIZE - 1>
makeStringLiteral(const char (&literal)[SIZE]) {
  return {literal};
}
...
constexpr auto makeMemberInfo(const char (&name)[NAME_SIZE], T U::*member,
                              Aliases &...aliases)
    -> MI<T, U,
          decltype(makeTuple(JS::Internal::makeStringLiteral(name),
                             JS::Internal::makeStringLiteral(aliases)...))> {
  return {makeTuple(JS::Internal::makeStringLiteral(name),
                    JS::Internal::makeStringLiteral(aliases)...),
          member};
}

template <typename T, size_t NAME_SIZE>
constexpr const Internal::SuperInfo<T>
makeSuperInfo(const char (&name)[NAME_SIZE]) {
...
template <typename T, typename Enable = void> struct TypeHandler {
  ...
  static inline void from(const T &from_type, Token &token,
                          Serializer &serializer);
};
...
inline bool
compareDataRefWithStringLiteral(const StringLiteral<STRINGSIZE> &memberName,
                                const DataRef &jsonName) {
  return jsonName.size == STRINGSIZE &&
         memcmp(memberName.data, jsonName.data, STRINGSIZE) == 0;
}
...
inline Error unpackMember(T &to_type,
                          const MemberInfo<MI_T, MI_M, MI_NC> &memberInfo,
                          ParseContext &context, size_t index, bool primary,
                          bool *assigned_members) {
  if (primary) {
    if (compareDataRefWithStringLiteral(memberInfo.names.template get<0>(),
                                        context.token.name)) {
      assigned_members[index] = true;
      return TypeHandler<MI_T>::to(to_type.*memberInfo.member, context);
    }
  } else {
    if (NameChecker<MI_NC, MI_NC::size>::compare(memberInfo.names,
                                                 context.token.name)) {
      assigned_members[index] = true;
      return TypeHandler<MI_T>::to(to_type.*memberInfo.member, context);
    }
...
inline Error verifyMember(const MemberInfo<MI_T, MI_M, MI_NC> &memberInfo,
                          size_t index, bool *assigned_members,
                          bool track_missing_members,
                          std::vector<std::string> &missing_members,
  ...
  if (track_missing_members) {
    std::string to_push =
        strlen(super_name) ? std::string(super_name) + "::" : std::string();
    to_push += std::string(memberInfo.names.template get<0>().data,
                           memberInfo.names.template get<0>().size);
...
template <typename T, typename MI_T, typename MI_M, typename MI_NC>
inline void serializeMember(const T &from_type,
                            const MemberInfo<MI_T, MI_M, MI_NC> &memberInfo,
...
template <typename T, size_t PAGE, size_t INDEX> struct SuperClassHandler {
  static Error handleSuperClasses(T &to_type, ParseContext &context,
                                  bool primary, bool *assigned_members);
  static Error verifyMembers(bool *assigned_members, bool track_missing_members,
                             std::vector<std::string> &missing_members);
  static constexpr size_t membersInSuperClasses();
  static void serializeMembers(const T &from_type, Token &token,
                               Serializer &serializer);
};

template <typename T, size_t PAGE, size_t SIZE> struct StartSuperRecursion {
  static Error start(T &to_type, ParseContext &context, bool primary,
                     bool *assigned) {
    return SuperClassHandler<T, PAGE, SIZE - 1>::handleSuperClasses(
        to_type, context, primary, assigned);
  }
  ...
  static void serializeMembers(const T &from_type, Token &token,
                               Serializer &serializer) {
    return SuperClassHandler<T, PAGE, SIZE - 1>::serializeMembers(
        from_type, token, serializer);
  }
};

template <typename T, size_t PAGE> constexpr size_t memberCount() {
  using Members = decltype(Internal::template JsonStructBaseDummy<
                           T, T>::js_static_meta_data_info());
  using SuperMeta = decltype(Internal::template JsonStructBaseDummy<
                             T, T>::js_static_meta_super_info());
  return Members::size +
         StartSuperRecursion<T, PAGE + Members::size,
                             SuperMeta::size>::membersInSuperClasses();
}
...
struct MemberChecker {
  inline static Error unpackMembers(T &to_type, const Members &members,
                                    ParseContext &context, bool primary,
                                    bool *assigned_members) {
    Error error = unpackMember(to_type, members.template get<INDEX>(), context,
                               PAGE + INDEX, primary, assigned_members);
    if (error != Error::MissingPropertyMember)
      return error;
  ...
  inline static Error verifyMembers(const Members &members,
                                    bool *assigned_members,
                                    bool track_missing_members,
                                    std::vector<std::string> &missing_members,
                                    const char *super_name) {
    Error memberError = verifyMember(
        members.template get<INDEX>(), PAGE + INDEX, assigned_members,
        track_missing_members, missing_members, super_name);
    Error error = MemberChecker<T, Members, PAGE, INDEX - 1>::verifyMembers(
        members, assigned_members, track_missing_members, missing_members,
        super_name);
    if (memberError != Error::NoError)
      return memberError;
    return error;
  }
  inline static void serializeMembers(const T &from_type,
                                      const Members &members, Token &token,
                                      Serializer &serializer,
                                      const char *super_name) {
    serializeMember(from_type,
                    members.template get<Members::size - INDEX - 1>(), token,
                    serializer, super_name);
    MemberChecker<T, Members, PAGE, INDEX - 1>::serializeMembers(
        from_type, members, token, serializer, super_name);
  }
};
...
struct MemberChecker<T, Members, PAGE, 0> {
  inline static Error unpackMembers(T &to_type, const Members &members,
                                    ParseContext &context, bool primary,
                                    bool *assigned_members) {
    Error error = unpackMember(to_type, members.template get<0>(), context,
                               PAGE, primary, assigned_members);
    if (error != Error::MissingPropertyMember)
      return error;

    using Super = decltype(Internal::template JsonStructBaseDummy<
                           T, T>::js_static_meta_super_info());
  ...
  inline static Error verifyMembers(const Members &members,
                                    ...
                                    const char *super_name) {
    Error memberError =
        verifyMember(members.template get<0>(), PAGE, assigned_members,
                     track_missing_members, missing_members, super_name);
    using Super = decltype(Internal::template JsonStructBaseDummy<
                           T, T>::js_static_meta_super_info());
    Error superError =
        StartSuperRecursion<T, PAGE + Members::size,
                            Super::size>::verifyMembers(assigned_members,
                                                        track_missing_members,
                                                        missing_members);
    if (memberError != Error::NoError)
      return memberError;
    return superError;
  ...
  inline static void serializeMembers(const T &from_type,
                                      ...
                                      Serializer &serializer,
                                      const char *super_name) {
    serializeMember(from_type, members.template get<Members::size - 1>(), token,
                    serializer, super_name);
    using Super = decltype(Internal::template JsonStructBaseDummy<
                           T, T>::js_static_meta_super_info());
    StartSuperRecursion<T, PAGE + Members::size, Super::size>::serializeMembers(
        from_type, token, serializer);
  }
};
...
Error SuperClassHandler<T, PAGE, INDEX>::handleSuperClasses(
  ...
  using Members = decltype(Internal::template JsonStructBaseDummy<
                           Super, Super>::js_static_meta_data_info());
  auto members =
      Internal::template JsonStructBaseDummy<Super,
                                             Super>::js_static_meta_data_info();
  Error error =
      MemberChecker<Super, Members, PAGE, Members::size - 1>::unpackMembers(
          static_cast<Super &>(to_type), members, context, primary,
          assigned_members);
  if (error != Error::MissingPropertyMember)
    return error;
  return SuperClassHandler<T, PAGE + memberCount<Super, 0>(),
                           INDEX - 1>::handleSuperClasses(to_type, context,
                                                          primary,
                                                          assigned_members);
}
...
Error SuperClassHandler<T, PAGE, INDEX>::verifyMembers(
    bool *assigned_members, bool track_missing_members,
    std::vector<std::string> &missing_members) {
  using SuperMeta = decltype(Internal::template JsonStructBaseDummy<
                             T, T>::js_static_meta_super_info());
  ...
  auto members =
      Internal::template JsonStructBaseDummy<Super,
                                             Super>::js_static_meta_data_info();
  const char *super_name =
      Internal::template JsonStructBaseDummy<T, T>::js_static_meta_super_info()
          .template get<INDEX>()
          .name.data;
  Error error =
      MemberChecker<Super, Members, PAGE, Members::size - 1>::verifyMembers(
          members, assigned_members, track_missing_members, missing_members,
          super_name);
  Error superError =
      SuperClassHandler<T, PAGE + memberCount<Super, 0>(),
                        INDEX - 1>::verifyMembers(assigned_members,
                                                  track_missing_members,
                                                  missing_members);
  if (error != Error::NoError)
    return error;
...
size_t constexpr SuperClassHandler<T, PAGE, INDEX>::membersInSuperClasses() {
  ...
  return memberCount<Super, PAGE>() +
         SuperClassHandler<T, PAGE + memberCount<Super, PAGE>(),
                           INDEX - 1>::membersInSuperClasses();
}

template <typename T, size_t PAGE, size_t INDEX>
void SuperClassHandler<T, PAGE, INDEX>::serializeMembers(
    const T &from_type, Token &token, Serializer &serializer) {
  using SuperMeta = decltype(Internal::template JsonStructBaseDummy<
                             T, T>::js_static_meta_super_info());
  using Super = typename TypeAt<INDEX, SuperMeta>::type::type;
  using Members = decltype(Internal::template JsonStructBaseDummy<
  ...
  SuperClassHandler<T, PAGE + memberCount<Super, 0>(),
                    INDEX - 1>::serializeMembers(from_type, token, serializer);
}

template <typename T, size_t PAGE> struct SuperClassHandler<T, PAGE, 0> {
  static Error handleSuperClasses(T &to_type, ParseContext &context,
                                  bool primary, bool *assigned_members) {
    using Meta = decltype(Internal::template JsonStructBaseDummy<
                          T, T>::js_static_meta_super_info());
    using Super = typename TypeAt<0, Meta>::type::type;
    using Members = decltype(Internal::template JsonStructBaseDummy<
                             Super, Super>::js_static_meta_data_info());
    auto members = Internal::template JsonStructBaseDummy<
        Super, Super>::js_static_meta_data_info();
    return MemberChecker<Super, Members, PAGE, Members::size - 1>::
        unpackMembers(static_cast<Super &>(to_type), members, context, primary,
                      assigned_members);
  }
  static Error verifyMembers(bool *assigned_members, bool track_missing_members,
                             std::vector<std::string> &missing_members) {
    ...
    auto members = Internal::template JsonStructBaseDummy<
        Super, Super>::js_static_meta_data_info();
    const char *super_name =
        Internal::template JsonStructBaseDummy<T,
                                               T>::js_static_meta_super_info()
            .template get<0>()
            .name.data;
    return MemberChecker<Super, Members, PAGE, Members::size - 1>::
        verifyMembers(members, assigned_members, track_missing_members,
  ...
  static void serializeMembers(const T &from_type, Token &token,
    ...
    using Members = decltype(Internal::template JsonStructBaseDummy<
                             Super, Super>::js_static_meta_data_info());
    auto members =
        Internal::JsonStructBaseDummy<Super, Super>::js_static_meta_data_info();
    MemberChecker<Super, Members, PAGE, Members::size - 1>::serializeMembers(
        from_type, members, token, serializer, "");
  }
};
...
static bool skipArrayOrObject(ParseContext &context) {
  ...
  return context.token.value_type == end_type &&
         context.error == Error::NoError;
}
} // namespace Internal

template <typename T> inline Error ParseContext::parseTo(T &to_type) {
  ...
  if (error != JS::Error::NoError &&
      tokenizer.errorContext().error == JS::Error::NoError) {
...
template <typename T> std::string serializeStruct(const T &from_type) {
...
template <> struct TypeHandler<Error> {
  ...
  static inline void from(const Error &from_type, Token &token,
                          Serializer &serializer) {
...
struct CallFunctionExecutionState {
  ...
  JS_OBJECT(JS_MEMBER(name), JS_MEMBER(context), JS_MEMBER(error),
            JS_MEMBER(error_string), JS_MEMBER(missing_members),
...
struct CallFunctionContext {
  CallFunctionContext(ParseContext &parser_context,
                      Serializer &return_serializer)
      : parse_context(parser_context), return_serializer(return_serializer),
        error_context(*this) {}
...
inline Error
CallFunctionErrorContext::setError(Error error,
                                   const std::string &errorString) {
  context.parse_context.error = error;
  if (context.execution_list.size()) {
    context.execution_list.back().error = error;
    context.execution_list.back().error_string.data =
        context.parse_context.tokenizer.makeErrorString();
  }
...
template <typename T, typename Ret, typename Arg, size_t NAME_SIZE,
          typename... Aliases>
constexpr FunctionInfo<T, Ret, Arg, sizeof...(Aliases) + 1, 0>
makeFunctionInfo(const char (&name)[NAME_SIZE], Ret (T::*function)(Arg),
                 Aliases... aliases) {
...
template <typename T, typename Ret, typename Arg, size_t NAME_SIZE,
          typename... Aliases>
constexpr FunctionInfo<T, Ret, Arg, sizeof...(Aliases) + 1, 1>
makeFunctionInfo(const char (&name)[NAME_SIZE],
                 Ret (T::*function)(Arg, CallFunctionErrorContext &),
                 Aliases... aliases) {
  return {{DataRef(name), DataRef(aliases)...}, function};
}
...
template <typename T, typename Ret, typename Arg, size_t NAME_SIZE,
          typename... Aliases>
constexpr FunctionInfo<T, Ret, Arg, sizeof...(Aliases) + 1, 2>
makeFunctionInfo(const char (&name)[NAME_SIZE],
                 Ret (T::*function)(Arg, CallFunctionContext &),
                 Aliases... aliases) {
  return {{DataRef(name), DataRef(aliases)...}, function};
}
...
constexpr FunctionInfo<T, Ret, void, sizeof...(Aliases) + 1, 0>
makeFunctionInfo(const char (&name)[NAME_SIZE], Ret (T::*function)(void),
                 Aliases... aliases) {
  return {{DataRef(name), DataRef(aliases)...}, function};
}
...
/// \private
template <typename T, typename Ret, size_t NAME_SIZE, typename... Aliases>
constexpr FunctionInfo<T, Ret, void, sizeof...(Aliases) + 1, 1>
makeFunctionInfo(const char (&name)[NAME_SIZE],
                 Ret (T::*function)(CallFunctionErrorContext &),
                 Aliases... aliases) {
  return {{DataRef(name), DataRef(aliases)...}, function};
}
...
/// \private
template <typename T, typename Ret, size_t NAME_SIZE, typename... Aliases>
constexpr FunctionInfo<T, Ret, void, sizeof...(Aliases) + 1, 2>
makeFunctionInfo(const char (&name)[NAME_SIZE],
                 Ret (T::*function)(CallFunctionContext &),
                 Aliases... aliases) {
  return {{DataRef(name), DataRef(aliases)...}, function};
}
...
template <typename T> struct HasJsonStructFunctionContainer {
  ...
  template <typename C>
  static constexpr yes &
  test_in_base(typename C::template JsonStructFunctionContainer<C> *);
...
struct JsonStructFunctionContainerDummy {
  using TT =
      decltype(JS_CONTAINER_STRUCT_T::template JsonStructFunctionContainer<
...
#define JS_FUNCTION(name)                                                      \
  JS::makeFunctionInfo(#name, &JS_CONTAINER_STRUCT_T::name)
#define JS_FUNCTION_ALIASES(name, ...)                                         \
  JS::makeFunctionInfo(#name, &JS_CONTAINER_STRUCT_T::name, __VA_ARGS__)
#define JS_FUNCTION_WITH_NAME(member, name)                                    \
  JS::makeFunctionInfo(name, &JS_CONTAINER_STRUCT_T::member)
#define JS_FUNCTION_WITH_NAME_ALIASES(member, name, ...)                       \
  JS::makeFunctionInfo(name, &JS_CONTAINER_STRUCT_T::member, __VA_ARGS__)

#define JS_INTERNAL_MAP_APPLY_FUNCTION(m, first)                               \
  m(#first, &JS_CONTAINER_STRUCT_T::first)
...
#define JS_INTERNAL_MAKE_FUNCTIONS(...)                                        \
  JS_INTERNAL_IF_ELSE(JS_INTERNAL_HAS_MORE_THAN_ONE_ARGS(__VA_ARGS__))         \
  (JS_INTERNAL_EXPAND(JS_INTERNAL_EVAL(                                        \
      JS_INTERNAL_MAP_FUNCTION(JS::makeFunctionInfo, __VA_ARGS__))))(          \
...
#define JS_FUNCTION_CONTAINER_EXTERNAL_INTERNAL_IMPL(Type, super_list,         \
                                                     function_list)            \
  namespace JS {                                                               \
  namespace Internal {                                                         \
  template <typename JS_CONTAINER_STRUCT_T>                                    \
...
#define JS_FUNC_OBJ(...)                                                       \
  JS_FUNCTION_CONTAINER_INTERNAL_IMPL(                                         \
      JS::makeTuple(), JS::makeTuple(JS_INTERNAL_MAKE_FUNCTIONS(__VA_ARGS__)))
#define JS_FUNCTION_CONTAINER(...)                                             \
  JS_FUNCTION_CONTAINER_INTERNAL_IMPL(JS::makeTuple(),                         \
                                      JS::makeTuple(__VA_ARGS__))
#define JS_FUNC_OBJ_SUPER(super_list, ...)                                     \
  JS_FUNCTION_CONTAINER_INTERNAL_IMPL(                                         \
      super_list, JS::makeTuple(JS_INTERNAL_MAKE_FUNCTIONS(__VA_ARGS__)))
...
#define JS_FUNCTION_CONTAINER_EXTERNAL_WITH_SUPER(Type, super_list, ...)       \
  JS_FUNCTION_CONTAINER_EXTERNAL_INTERNAL_IMPL(Type, super_list,               \
                                               JS::makeTuple(__VA_ARGS__))

#define JS_FUNCTION_CONTAINER_EXTERNAL_WITH_SUPER_WITHOUT_MEMBERS(Type,        \
                                                                  super_list)  \
  JS_FUNCTION_CONTAINER_EXTERNAL_INTERNAL_IMPL(Type, super_list,               \
                                               JS::makeTuple())
namespace Internal {
...
struct FunctionCaller {
  static Error callFunctionAndSerializeReturn(
    ...
    PureArg arg;
    context.parse_context.error =
        TypeHandler<PureArg>::to(arg, context.parse_context);
    if (context.parse_context.error != Error::NoError)
      return context.parse_context.error;

    Token token;
    TypeHandler<Ret>::from((container.*functionInfo.function)(arg), token,
                           context.return_serializer);
    return Error::NoError;
  }
};
...
struct FunctionCaller<T, U, Ret, Arg, NAME_COUNT, 1> {
  static Error callFunctionAndSerializeReturn(
      T &container, FunctionInfo<U, Ret, Arg, NAME_COUNT, 1> &functionInfo,
      CallFunctionContext &context) {
    typedef typename std::remove_reference<Arg>::type NonRefArg;
    typedef typename std::remove_cv<NonRefArg>::type PureArg;
    PureArg arg;
...
struct FunctionCaller<T, U, Ret, Arg, NAME_COUNT, 2> {
  static Error callFunctionAndSerializeReturn(
      T &container, FunctionInfo<U, Ret, Arg, NAME_COUNT, 2> &functionInfo,
      CallFunctionContext &context) {
    typedef typename std::remove_reference<Arg>::type NonRefArg;
    typedef typename std::remove_cv<NonRefArg>::type PureArg;
    PureArg arg;
...
struct FunctionCaller<T, U, void, Arg, NAME_COUNT, 1> {
  static Error callFunctionAndSerializeReturn(
      T &container, FunctionInfo<U, void, Arg, NAME_COUNT, 1> &functionInfo,
      CallFunctionContext &context) {
    typedef typename std::remove_reference<Arg>::type NonRefArg;
    typedef typename std::remove_cv<NonRefArg>::type PureArg;
    PureArg arg;
...
struct FunctionCaller<T, U, void, Arg, NAME_COUNT, 2> {
  static Error callFunctionAndSerializeReturn(
      T &container, FunctionInfo<U, void, Arg, NAME_COUNT, 2> &functionInfo,
      CallFunctionContext &context) {
    typedef typename std::remove_reference<Arg>::type NonRefArg;
    typedef typename std::remove_cv<NonRefArg>::type PureArg;
    PureArg arg;
...
struct FunctionCaller<T, U, Ret, void, NAME_COUNT, TAKES_CONTEXT> {
  static Error callFunctionAndSerializeReturn(
    ...
    if (context.parse_context.error != Error::NoError)
      return context.parse_context.error;
    Token token;
    TypeHandler<Ret>::from((container.*functionInfo.function)(), token,
                           context.return_serializer);
    return Error::NoError;
  }
};
...
struct FunctionCaller<T, U, Ret, void, NAME_COUNT, 1> {
  static Error callFunctionAndSerializeReturn(
      T &container, FunctionInfo<U, Ret, void, NAME_COUNT, 1> &functionInfo,
      CallFunctionContext &context) {
    checkValidVoidParameter(context);
    if (context.parse_context.error != Error::NoError)
      return context.parse_context.error;
...
struct FunctionCaller<T, U, Ret, void, NAME_COUNT, 2> {
  static Error callFunctionAndSerializeReturn(
      T &container, FunctionInfo<U, Ret, void, NAME_COUNT, 2> &functionInfo,
      CallFunctionContext &context) {
    checkValidVoidParameter(context);
    if (context.parse_context.error != Error::NoError)
      return context.parse_context.error;
...
struct FunctionCaller<T, U, void, void, NAME_COUNT, 1> {
  static Error callFunctionAndSerializeReturn(
      T &container, FunctionInfo<U, void, void, NAME_COUNT, 1> &functionInfo,
      CallFunctionContext &context) {
    checkValidVoidParameter(context);
    if (context.parse_context.error != Error::NoError)
      return context.parse_context.error;
...
struct FunctionCaller<T, U, void, void, NAME_COUNT, 2> {
  static Error callFunctionAndSerializeReturn(
      T &container, FunctionInfo<U, void, void, NAME_COUNT, 2> &functionInfo,
      CallFunctionContext &context) {
    checkValidVoidParameter(context);
    if (context.parse_context.error != Error::NoError)
      return context.parse_context.error;
...
Error matchAndCallFunction(
    T &container, CallFunctionContext &context,
    FunctionInfo<U, Ret, Arg, NAME_COUNT, TAKES_CONTEXT> &functionInfo,
    bool primary) {
  if (primary &&
      context.parse_context.token.name.size == functionInfo.name[0].size &&
      memcmp(functionInfo.name[0].data, context.parse_context.token.name.data,
             functionInfo.name[0].size) == 0) {
    return Internal::FunctionCaller<T, U, Ret, Arg, NAME_COUNT, TAKES_CONTEXT>::
        callFunctionAndSerializeReturn(container, functionInfo, context);
  } else if (!primary) {
    for (size_t i = 1; i < NAME_COUNT; i++) {
      if (context.parse_context.token.name.size == functionInfo.name[i].size &&
          memcmp(functionInfo.name[i].data,
                 context.parse_context.token.name.data,
                 functionInfo.name[i].size) == 0) {
        return Internal::FunctionCaller<
            T, U, Ret, Arg, NAME_COUNT,
            TAKES_CONTEXT>::callFunctionAndSerializeReturn(container,
                                                           functionInfo,
                                                           context);
      }
    }
...
template <typename T, size_t SIZE> struct StartFunctionalSuperRecursion {
  static Error callFunction(T &container, CallFunctionContext &context,
                            bool primary) {
    return FunctionalSuperRecursion<T, SIZE - 1>::callFunction(
...
struct FunctionObjectTraverser {
  static Error call(T &container, CallFunctionContext &context,
                    Functions &functions, bool primary) {
    ...
    if (error != Error::MissingFunction)
      return context.parse_context.error;
    return FunctionObjectTraverser<T, Functions, INDEX - 1>::call(
...
struct FunctionObjectTraverser<T, Functions, 0> {
  static Error call(T &container, CallFunctionContext &context,
    ...
    if (error != Error::MissingFunction)
      return error;
    using SuperMeta =
        decltype(Internal::template JsonStructFunctionContainerDummy<
                 T, T>::js_static_meta_super_info());
    return StartFunctionalSuperRecursion<T, SuperMeta::size>::callFunction(
        container, context, primary);
  }
};
...
struct FunctionObjectTraverser<T, Functions, size_t(-1)> {
  static Error call(T &container, CallFunctionContext &context, Functions &,
                    bool primary) {
    using SuperMeta =
        decltype(Internal::template JsonStructFunctionContainerDummy<
                 T, T>::js_static_meta_super_info());
...
static inline void add_error(CallFunctionExecutionState &executionState,
  ...
  if (context.unassigned_required_members.size())
    std::swap(executionState.unassigned_required_members.data,
              context.unassigned_required_members);
}
} // namespace Internal
...
inline Error CallFunctionContext::callFunctions(T &container) {
  beforeCallFunctions();
  Internal::RAICallFunctionOnExit callOnExit(
      *this, &CallFunctionContext::afterCallFunctions);
  JS::Error error = parse_context.nextToken();
  if (error != JS::Error::NoError)
    return error;
  if (parse_context.token.value_type != JS::Type::ObjectStart) {
    return error_context.setError(
        Error::ExpectedObjectStart,
        "Can only call functions on objects with members");
  }
  ...
  auto &functions = Internal::JsonStructFunctionContainerDummy<
      T, T>::js_static_meta_functions_info();
  using FunctionsType =
      typename std::remove_reference<decltype(functions)>::type;
  while (parse_context.token.value_type != JS::Type::ObjectEnd) {
    parse_context.tokenizer.pushScope(parse_context.token.value_type);
    execution_list.push_back(CallFunctionExecutionState(std::string(
        parse_context.token.name.data, parse_context.token.name.size)));
    execution_list.back().context.data = user_context;
    error = Internal::FunctionObjectTraverser<
        T, FunctionsType, FunctionsType::size - 1>::call(container, *this,
                                                         functions, true);
    if (error == Error::MissingFunction)
      error = Internal::FunctionObjectTraverser<
          T, FunctionsType, FunctionsType::size - 1>::call(container, *this,
                                                           functions, false);
    if (error != Error::NoError) {
      assert(error == parse_context.error ||
             parse_context.error == Error::NoError);
...
struct DefaultCallFunctionContext : public CallFunctionContext {
  ...
  DefaultCallFunctionContext(const char (&data)[SIZE], std::string &json_out)
      : CallFunctionContext(p_context, s_context.serializer), p_context(data),
        s_context(json_out) {}
...
Error FunctionalSuperRecursion<T, INDEX>::callFunction(
    T &container, CallFunctionContext &context, bool primary) {
  using SuperMeta =
      decltype(Internal::template JsonStructFunctionContainerDummy<
               T, T>::js_static_meta_super_info());
  using Super = typename TypeAt<INDEX, SuperMeta>::type::type;
  auto &functions = Internal::template JsonStructFunctionContainerDummy<
      Super, Super>::js_static_meta_functions_info();
  using FunctionsType =
      typename std::remove_reference<decltype(functions)>::type;
  Error error =
      FunctionObjectTraverser<Super, FunctionsType,
                              FunctionsType::size - 1>::call(container, context,
                                                             functions,
                                                             primary);
  if (error != Error::MissingFunction)
    return error;

  return FunctionalSuperRecursion<T, INDEX - 1>::callFunction(container,
                                                              context, primary);
}
...
template <typename T> struct FunctionalSuperRecursion<T, 0> {
  static Error callFunction(T &container, CallFunctionContext &context,
    ...
    using FunctionsType =
        typename std::remove_reference<decltype(functions)>::type;
    return FunctionObjectTraverser<Super, FunctionsType,
                                   FunctionsType::size - 1>::call(container,
                                                                  context,
                                                                  functions,
                                                                  primary);
  }
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    case ParseEnumStringState::FindingNameEnd:
      if (c == '\0' || c == '\t' || c == '\n' || c == '\r' || c == ' ' ||
        ...
        state = c == ',' ? ParseEnumStringState::FindingNameStart
                         : ParseEnumStringState::FindingSeperator;
      }
      break;
...
#define JS_ENUM_DECLARE_STRING_PARSER(name)                                    \
  ...
  template <> struct TypeHandler<name> {                                       \
    static inline Error to(name &to_type, ParseContext &context) {             \
      return Internal::EnumHandler<name, js_##name##_string_struct>::to(       \
          to_type, context);                                                   \
    }                                                                          \
    static inline void from(const name &from_type, Token &token,               \
                            Serializer &serializer) {                          \
      return Internal::EnumHandler<name, js_##name##_string_struct>::from(     \
          from_type, token, serializer);                                       \
    }                                                                          \
  };                                                                           \
  }
...
#define JS_ENUM_NAMESPACE_DECLARE_STRING_PARSER(ns, name)                      \
  ...
  template <> struct TypeHandler<ns::name> {                                   \
    static inline Error to(ns::name &to_type, ParseContext &context) {         \
      return Internal::EnumHandler<                                            \
          ns::name, ns::js_##name##_string_struct>::to(to_type, context);      \
    }                                                                          \
    static inline void from(const ns::name &from_type, Token &token,           \
                            Serializer &serializer) {                          \
      return Internal::EnumHandler<                                            \
          ns::name, ns::js_##name##_string_struct>::from(from_type, token,     \
                                                         serializer);          \
    }                                                                          \
  };                                                                           \
  }
...
inline Error TypeHandler<T, Enable>::to(T &to_type, ParseContext &context) {
...

  {
    ...
    error = Internal::MemberChecker<T, MembersType, 0, MembersType::size - 1>::
        unpackMembers(to_type, members, context, true, assigned_members);
    if (error == Error::MissingPropertyMember)
      error =
          Internal::MemberChecker<T, MembersType, 0, MembersType::size - 1>::
              unpackMembers(to_type, members, context, false, assigned_members);
    if (error == Error::MissingPropertyMember) {

      if (context.track_member_assignement_state)
        context.missing_members.emplace_back(token_name.data,
                                             token_name.data + token_name.size);
  ...
  std::vector<std::string> unassigned_required_members;
  error = Internal::MemberChecker<T, MembersType, 0, MembersType::size - 1>::
      verifyMembers(members, assigned_members,
                    context.track_member_assignement_state,
                    unassigned_required_members, "");
  if (error == Error::UnassignedRequiredMember) {
    if (context.track_member_assignement_state)
      context.unassigned_required_members.insert(
          context.unassigned_required_members.end(),
...
template <typename T, typename Enable>
void TypeHandler<T, Enable>::from(const T &from_type, Token &token,
                                  Serializer &serializer) {
  ...
  auto members =
      Internal::JsonStructBaseDummy<T, T>::js_static_meta_data_info();
  using MembersType = decltype(members);
  Internal::MemberChecker<T, MembersType, 0,
                          MembersType::size - 1>::serializeMembers(from_type,
                                                                   members,
                                                                   token,
                                                                   serializer,
                                                                   "");
...
static void push_back_escape(char current_char, std::string &to_type) {
  static const char escaped_table[] = {'b', 'f',  'n',  'r',
                                       't', '\"', '\\', '/'};
  static const char replace_table[] = {'\b', '\f', '\n', '\r',
                                       '\t', '\"', '\\', '/'};
  static_assert(sizeof(escaped_table) == sizeof(replace_table),
                "Static tables have to be the same.");
  const char *it = static_cast<const char *>(
      memchr(escaped_table, current_char, sizeof(escaped_table)));
  if (it) {
...
static void handle_json_escapes_in(const DataRef &ref, std::string &to_type) {
  ...
  while (size) {
    ...
    // we assume utf-8 encoding when this notation is used and parsing into
    // std::string
...
static DataRef handle_json_escapes_out(const std::string &data,
                                       std::string &buffer) {
  int start_index = 0;
  for (size_t i = 0; i < data.size(); i++) {
    const char cur = data[i];
    if (static_cast<uint8_t>(cur) <= uint8_t('\r') || cur == '\"' ||
        cur == '\\') {
      if (buffer.empty()) {
      ...
      if (diff > 0) {
        buffer.insert(buffer.end(), data.data() + start_index,
                      data.data() + start_index + diff);
      }
      start_index = int(i) + 1;
...
template <> struct TypeHandler<std::string> {
  ...
  static inline void from(const std::string &str, Token &token,
                          Serializer &serializer) {
    std::string buffer;
...
template <int shift = 1> inline void left_shift(uint64_t (&a)[2]) {
  static_assert(shift < sizeof(*a) * 8,
                "This functions does only support shifting by sizes smaller "
                "than sizeof(*a) * 8");
...
template <> struct float_info<double> {
  ...
  conversion_type_has_mask(const str_to_float_conversion_type &a) noexcept {
    return a[1] & str_to_float_mask();
  }
  static inline constexpr bool conversion_type_has_top_bit_in_mask(
      const str_to_float_conversion_type &a) noexcept {
    return a[1] & str_to_float_top_bit_in_mask();
  }
  static inline constexpr bool
  conversion_type_is_null(const str_to_float_conversion_type &a) noexcept {
    return !a[0] && !a[1];
  }
  static inline int shift_left_msb_to_index(str_to_float_conversion_type &a,
                                            int index) {
    if (a[1]) {
  ...
  static inline void
  copy_denormal_to_type(const str_to_float_conversion_type &a,
                        int binary_exponent, bool negative, double &to_digit) {
    uint64_t q = a[1];
  ...
  static inline void copy_normal_to_type(const str_to_float_conversion_type &a,
                                         int binary_exponent, bool negative,
                                         double &to_digit) {
    uint64_t q = a[1] & ~str_to_float_mask();
    uint64_t to_round_off = (q & ((uint64_t(1) << 8) - 1));
    bool bigger = to_round_off > (uint64_t(1) << (8 - 1)) ||
                  (to_round_off == (uint64_t(1) << (8 - 1)) && a[0]);
    bool tie_odd =
        (!(q & ((uint64_t(1) << 7) - 1))) && (q & (uint64_t(1) << 8)) && !a[0];
    if (bigger || tie_odd) {
      q += uint64_t(1) << (8 - 1);
    }
    q >>= 8;
...
inline void get_parts(T f, bool &negative, int &exp, uint64_t &mentissa) {
  ...
  exp = int((bits >> float_info<T>::mentissa_width()) &
            (((uint64_t(1) << float_info<T>::exponent_width()) - 1)));
...
template <typename T>
inline void
assign_significand_to_float_conversion_type(const float_base10<T> &significand,
                                            uint64_t (&a)[2]) {
...
template <> struct float_info<float> {
  ...
  static inline void
  copy_denormal_to_type(const str_to_float_conversion_type &a,
                        int binary_exponent, bool negative, float &to_digit) {
    uint64_t q = a;
  ...
  static inline void copy_normal_to_type(const str_to_float_conversion_type &a,
    ...
    bool tie_odd =
        (!(q & ((uint64_t(1) << 36) - 1))) && (q & (uint64_t(1) << 37));
    if (bigger || tie_odd) {
      q += (uint64_t(1) << (37 - 1));
    }
    q >>= 37;
...
template <typename T> inline bool is_nan(T t) {
  ...
  return exp == ((int(1) << float_info<T>::exponent_width()) - 1) &&
         mentissa > 0;
}
...
struct StaticLog10 {
  constexpr static int get() noexcept {
    return StaticLog10<T, VALUE / 10, SUM + 1, ABORT_VALUE,
                       VALUE / 10 != ABORT_VALUE>::get();
  }
};
...
template <typename T, int WIDTH, int CURRENT> struct CharsInDigit {
  static int lower_bounds(T t) noexcept {
    if (Pow10<T, CURRENT + WIDTH / 2, 1>::get() - 1 < t) {
      return CharsInDigit<T, WIDTH - (WIDTH / 2 + 1),
                          CURRENT + WIDTH / 2 + 1>::lower_bounds(t);
    }
...
template <typename T> int count_chars(T t) noexcept {
  if (iabs<T>(t) < T(10))
    return 1;
  constexpr int maxChars =
      StaticLog10<T, std::numeric_limits<T>::max(), 0, 0, true>::get() + 1;
...
template <> struct cache_values<float> {
  ...
  static const uint64_t *less_than(int index) {
    static const uint64_t data[48] = {/*  0*/ UINT64_C(2305843009213693952),
                                      /*  1*/ UINT64_C(1441151880758558720),
                                      /*  2*/ UINT64_C(1801439850948198400),
                                      /*  3*/ UINT64_C(2251799813685248000),
                                      /*  4*/ UINT64_C(1407374883553280000),
                                      /*  5*/ UINT64_C(1759218604441600000),
                                      /*  6*/ UINT64_C(2199023255552000000),
                                      /*  7*/ UINT64_C(1374389534720000000),
                                      /*  8*/ UINT64_C(1717986918400000000),
                                      /*  9*/ UINT64_C(2147483648000000000),
                                      /* 10*/ UINT64_C(1342177280000000000),
                                      /* 11*/ UINT64_C(1677721600000000000),
                                      /* 12*/ UINT64_C(2097152000000000000),
                                      /* 13*/ UINT64_C(1310720000000000000),
                                      /* 14*/ UINT64_C(1638400000000000000),
                                      /* 15*/ UINT64_C(2048000000000000000),
                                      /* 16*/ UINT64_C(1280000000000000000),
                                      /* 17*/ UINT64_C(1600000000000000000),
                                      /* 18*/ UINT64_C(2000000000000000000),
                                      /* 19*/ UINT64_C(1250000000000000000),
                                      /* 20*/ UINT64_C(1562500000000000000),
                                      /* 21*/ UINT64_C(1953125000000000000),
                                      /* 22*/ UINT64_C(1220703125000000000),
                                      /* 23*/ UINT64_C(1525878906250000000),
                                      /* 24*/ UINT64_C(1907348632812500000),
                                      /* 25*/ UINT64_C(1192092895507812500),
                                      /* 26*/ UINT64_C(1490116119384765625),
                                      /* 27*/ UINT64_C(1862645149230957031),
                                      /* 28*/ UINT64_C(1164153218269348144),
                                      /* 29*/ UINT64_C(1455191522836685180),
                                      /* 30*/ UINT64_C(1818989403545856475),
                                      /* 31*/ UINT64_C(2273736754432320594),
                                      /* 32*/ UINT64_C(1421085471520200371),
                                      /* 33*/ UINT64_C(1776356839400250464),
                                      /* 34*/ UINT64_C(2220446049250313080),
                                      /* 35*/ UINT64_C(1387778780781445675),
                                      /* 36*/ UINT64_C(1734723475976807094),
                                      /* 37*/ UINT64_C(2168404344971008868),
                                      /* 38*/ UINT64_C(1355252715606880542),
                                      /* 39*/ UINT64_C(1694065894508600678),
                                      /* 40*/ UINT64_C(2117582368135750847),
                                      /* 41*/ UINT64_C(1323488980084844279),
                                      /* 42*/ UINT64_C(1654361225106055349),
                                      /* 43*/ UINT64_C(2067951531382569187),
                                      /* 44*/ UINT64_C(1292469707114105741),
                                      /* 45*/ UINT64_C(1615587133892632177),
                                      /* 46*/ UINT64_C(2019483917365790221),
                                      /* 47*/ UINT64_C(1262177448353618888)};
    return &data[index];
  }

  static const uint64_t *greater_than_equals(int index) {
    static const uint64_t data[30] = {/*  0*/ UINT64_C(576460752303423488),
                                      /*  1*/ UINT64_C(461168601842738790),
                                      /*  2*/ UINT64_C(368934881474191032),
                                      /*  3*/ UINT64_C(295147905179352825),
                                      /*  4*/ UINT64_C(472236648286964521),
                                      /*  5*/ UINT64_C(377789318629571617),
                                      /*  6*/ UINT64_C(302231454903657293),
                                      /*  7*/ UINT64_C(483570327845851669),
                                      /*  8*/ UINT64_C(386856262276681335),
                                      /*  9*/ UINT64_C(309485009821345068),
                                      /* 10*/ UINT64_C(495176015714152109),
                                      /* 11*/ UINT64_C(396140812571321687),
                                      /* 12*/ UINT64_C(316912650057057350),
                                      /* 13*/ UINT64_C(507060240091291760),
                                      /* 14*/ UINT64_C(405648192073033408),
                                      /* 15*/ UINT64_C(324518553658426726),
                                      /* 16*/ UINT64_C(519229685853482762),
                                      /* 17*/ UINT64_C(415383748682786210),
                                      /* 18*/ UINT64_C(332306998946228968),
                                      /* 19*/ UINT64_C(531691198313966349),
                                      /* 20*/ UINT64_C(425352958651173079),
                                      /* 21*/ UINT64_C(340282366920938463),
                                      /* 22*/ UINT64_C(544451787073501541),
                                      /* 23*/ UINT64_C(435561429658801233),
                                      /* 24*/ UINT64_C(348449143727040986),
                                      /* 25*/ UINT64_C(557518629963265578),
                                      /* 26*/ UINT64_C(446014903970612462),
                                      /* 27*/ UINT64_C(356811923176489970),
                                      /* 28*/ UINT64_C(570899077082383952),
                                      /* 29*/ UINT64_C(456719261665907161)};
    return &data[index];
  }
};
...
inline void compute_shortest(uint64_t a, uint64_t b, uint64_t c,
                             bool accept_smaller, bool accept_larger,
                             bool break_tie_down, int &exponent_adjuster,
  ...
  bool round_down =
      (want_to_round_down && (a != b || all_a_zero)) || (b + 1 > c);
  if (round_down) {
    shortest_base10 = b;
  } else {
    shortest_base10 = b + 1;
...
template <typename T>
inline uint64_t multiply_and_shift(uint64_t a, const uint64_t *b,
                                   int shift_right, bool round_up) {
  (void)a;
  (void)b;
  (void)shift_right;
  (void)round_up;
...
template <>
inline uint64_t multiply_and_shift<double>(uint64_t a, const uint64_t *b,
                                           int shift_right, bool round_up) {
  uint64_t a0, a1, b0, b1, b2, b3, a0b0, a0b1, a0b2, a0b3, a1b0, a1b1, a1b2,
      a1b3;
  a0 = low(a);
  a1 = high(a);
  b0 = low(b[0]);
  ...
  ret[index] |= (ret[index + 1] & ((uint64_t(1) << shift_right_in_index) - 1))
                << (64 - shift_right_in_index);
  ret[index] += round_up;
  return ret[index];
}

template <>
inline uint64_t multiply_and_shift<float>(uint64_t a, const uint64_t *b,
                                          int shift_right, bool round_up) {
...
static float_base10<SignificandType> decode(T f) {
  ...
  if (exp >= 0) {
    ...
    int k = int(std::ceil((double(-exp) - double(q)) * log_2_5)) -
            cache_values<T>::b1;
    shift_right = q - k;
  ...
  auto cache_value = exp >= 0 ? cache_values<T>::greater_than_equals(q)
                              : cache_values<T>::less_than(-exp - q);
  ...
  int exponent_adjust;
  uint64_t shortest_base10;
  compute_shortest(a, b, c, accept_smaller && zero[0],
                   accept_larger || !zero[2], zero[1], exponent_adjust,
                   shortest_base10);
  int significand_digit_count = count_chars(shortest_base10);
  int e = exponent_adjust + e10 + q;
  return {negative, false,          false, uint8_t(significand_digit_count),
          e,        shortest_base10};
}

template <typename T>
inline int convert_parsed_to_buffer(const float_base10<T> &result, char *buffer,
                                    int buffer_size, int max_expanded_length,
  ...
  int complete_digits =
      max(1, digits_before_decimals) + max(1, digits_after_decimals) + 1;
  if (complete_digits < max_expanded_length) {
    ...
    if (buffer_size < complete_digits) {
      ...
      if (to_remove > 0) {
        ...
        to_remove--;
        if (to_remove > 0) {
          int to_remove_before_decimals =
              std::min(to_remove, digits_before_decimals);
    ...
    for (int i = 0; i < result.significand_digit_count; i++) {
      significan_buffer[result.significand_digit_count - i - 1] =
          '0' + significand % 10;
      significand /= 10;
    }
    ...
    for (int i = 0; i < exponent_digit_count; i++) {
      exponent_buffer[exponent_digit_count + (exp < 0) - i - 1] =
          '0' + abs_exp % 10;
      abs_exp /= 10;
    }
    ...
    int to_copy =
        min(buffer_size - offset, int(result.significand_digit_count) - 1);
...
inline bool is_space(char a) {
  if (a == 0x20 || a == 0x09 || a == 0x0a || a == 0x0b || a == 0x0c ||
      a == 0x0d)
    return true;
  return false;
}
...
inline parse_string_error parseNumber(const char *number, size_t size,
                                      parsed_string<T> &parsedString) {
  const char *current;
  ...
  current =
      find_if(number, number_end, [](const char a) { return !is_space(a); });
  ...
  if (desimal_position >= 0)
    parsedString.exp =
        desimal_position - parsedString.significand_digit_count + exponent;
  else
...
inline T convertToNumber(const parsed_string<SignificandType> &parsed) {
  ...
  using uint_conversion_type =
      typename float_info<T>::str_to_float_conversion_type;
  ...
  if (binary_exponent <= 0) {
    float_info<T>::copy_denormal_to_type(a, binary_exponent, parsed.negative,
                                         to_digit);
  } else if (binary_exponent <
             (int(1) << float_info<T>::exponent_width()) - 1) {
    float_info<T>::copy_normal_to_type(a, binary_exponent, parsed.negative,
                                       to_digit);
  } else {
...
namespace ryu {
template <typename T>
int to_buffer(T d, char *buffer, int buffer_size,
              int *digits_truncated = nullptr) {
  auto decoded = decode<T, uint64_t>(d);
  return convert_parsed_to_buffer(decoded, buffer, buffer_size,
...
template <typename T> inline std::string to_string(T f) {
  ...
  ret.resize(25);
  ret.resize(size_t(
      convert_parsed_to_buffer(decoded, &ret[0], int(ret.size()),
                               float_info<T>::str_to_float_expanded_length())));
  return ret;
}
} // namespace ryu
...
namespace integer {
template <typename T>
inline int to_buffer(T integer, char *buffer, int buffer_size,
                     int *digits_truncated = nullptr) {
  static_assert(std::is_integral<T>::value,
                "Tryint to convert non int to string");
...
make_integer_return_value(SignificandType significand, bool negative) {
  return negative ? -T(significand) : T(significand);
}

template <typename T, typename SignificandType>
inline typename std::enable_if<std::is_unsigned<T>::value, T>::type
make_integer_return_value(SignificandType significand, bool) {
...
inline T convert_to_integer(const parsed_string<SignificandType> &parsed) {
  if (parsed.inf)
    return parsed.negative ? std::numeric_limits<T>::min()
  ...
  if (exp < 0) {
    ...
    int chars_in_sig = count_chars(significand);
    if (exp >
        ft::StaticLog10<T, std::numeric_limits<T>::max(), 0, 0, true>::get() -
            chars_in_sig)
      return parsed.negative ? std::numeric_limits<T>::min()
                             : std::numeric_limits<T>::max();
    while (exp > 0) {
...
template <typename T>
inline parse_string_error to_integer(const char *str, size_t size, T &target,
                                     const char *(&endptr)) {
...
template <typename T>
inline parse_string_error to_integer(const std::string &str, T &target,
                                     const char *(&endptr)) {
...
template <typename T>
inline parse_string_error to_ieee_t(const char *str, size_t size, T &target,
                                    const char *(&endptr)) {
...
inline parse_string_error to_float(const char *str, size_t size, float &target,
                                   const char *(&endptr)) {
  return to_ieee_t(str, size, target, endptr);
}

inline parse_string_error to_double(const char *str, size_t size,
                                    double &target, const char *(&endptr)) {
  return to_ieee_t(str, size, target, endptr);
}
...
template <> struct TypeHandler<double> {
  static inline Error to(double &to_type, ParseContext &context) {
    const char *pointer;
    auto result = Internal::ft::to_double(
        context.token.value.data, context.token.value.size, to_type, pointer);
  ...
  static inline void from(const double &d, Token &token,
                          Serializer &serializer) {
    // char buf[1/*'-'*/ + (DBL_MAX_10_EXP+1)/*308+1 digits*/ + 1/*'.'*/ +
    // 6/*Default? precision*/ + 1/*\0*/];
    char buf[32];
    int size;
...
template <> struct TypeHandler<float> {
  static inline Error to(float &to_type, ParseContext &context) {
    const char *pointer;
    auto result = Internal::ft::to_float(
        context.token.value.data, context.token.value.size, to_type, pointer);
    if (result != Internal::ft::parse_string_error::ok ||
        context.token.value.data + context.token.value.size != pointer)
  ...
  static inline void from(const float &f, Token &token,
                          Serializer &serializer) {
    char buf[16];
    int size;
...
template <typename T> struct TypeHandlerIntType {
  static inline Error to(T &to_type, ParseContext &context) {
    const char *pointer;
    auto parse_error = Internal::ft::integer::to_integer(
        context.token.value.data, context.token.value.size, to_type, pointer);
  ...
  static inline void from(const T &from_type, Token &token,
    ...
    int size = Internal::ft::integer::to_buffer(from_type, buf, sizeof(buf),
                                                &digits_truncated);
...
/// \private
template <>
struct TypeHandler<unsigned short int>
    : TypeHandlerIntType<unsigned short int> {};
...
/// \private
template <>
struct TypeHandler<unsigned long long int>
    : TypeHandlerIntType<unsigned long long int> {};
...
public:
  ...
  static inline void from(const Nullable<T> &opt, Token &token,
                          Serializer &serializer) {
...
public:
  ...
  static inline void from(const NullableChecked<T> &opt, Token &token,
                          Serializer &serializer) {
    if (opt.null) {
...
public:
  ...
  static inline void from(const Optional<T> &opt, Token &token,
                          Serializer &serializer) {
    TypeHandler<T>::from(opt(), token, serializer);
  }
};
...
public:
  ...
  static inline void from(const OptionalChecked<T> &opt, Token &token,
                          Serializer &serializer) {
    if (opt.assigned)
      TypeHandler<T>::from(opt(), token, serializer);
  }
...
public:
  ...
  static inline void from(const std::optional<T> &opt, Token &token,
                          Serializer &serializer) {
    if (opt.has_value())
...
public:
  ...
  static inline void from(const std::shared_ptr<T> &unique, Token &token,
                          Serializer &serializer) {
    if (unique) {
...
public:
  ...
  static inline void from(const std::unique_ptr<T> &unique, Token &token,
                          Serializer &serializer) {
    if (unique) {
      TypeHandler<T>::from(*unique.get(), token, serializer);
    } else {
  ...
  static inline Error to(bool &to_type, ParseContext &context) {
    if (context.token.value.size == sizeof("true") - 1 &&
      ...
      to_type = true;
             ...
             memcmp("false", context.token.value.data, sizeof("false") - 1) ==
                 0)
      to_type = false;
    else
...
/// \private
template <class T>
struct TypeHandler<T, typename std::enable_if_t<Internal::is_specialization<
                          T, std::chrono::time_point>::value>> {
  static inline Error to(T &to_type, ParseContext &context) {
    uint64_t t;
  ...
  static inline void from(const T &val, Token &token, Serializer &serializer) {
    uint64_t t;
    if constexpr (std::is_same_v<std::chrono::high_resolution_clock::time_point,
                                 T>)
...
/// \private
template <typename T> struct TypeHandler<std::vector<T>> {
  ...
  static inline void from(const std::vector<T> &vec, Token &token,
                          Serializer &serializer) {
    token.value_type = Type::ArrayStart;
    token.value = DataRef("[");
    serializer.write(token);
...
/// \private
template <> struct TypeHandler<std::vector<bool>> {
public:
  ...
  static inline void from(const std::vector<bool> &vec, Token &token,
                          Serializer &serializer) {
    token.value_type = Type::ArrayStart;
    token.value = DataRef("[");
    serializer.write(token);
...
template <> struct TypeHandler<SilentString> {
  ...
  static inline void from(const SilentString &str, Token &token,
                          Serializer &serializer) {
    if (str.data.size()) {
...
/// \private
template <typename T> struct TypeHandler<SilentVector<T>> {
public:
  ...
  static inline void from(const SilentVector<T> &vec, Token &token,
                          Serializer &serializer) {
    if (vec.data.size()) {
...
public:
  ...
  static inline void from(const SilentUniquePtr<T> &ptr, Token &token,
                          Serializer &serializer) {
    if (ptr.data) {
...
/// \private
template <> struct TypeHandler<std::vector<Token>> {
public:
  static inline Error to(std::vector<Token> &to_type, ParseContext &context) {
    ...
    auto ref = context.tokenizer.registerNeedMoreDataCallback(
        [&buffer_change](JS::Tokenizer &tokenizer) {
          JS_UNUSED(tokenizer);
          buffer_change = true;
        });
    ...
    while (error == JS::Error::NoError && level && buffer_change == false) {
      ...
      if (context.token.value_type == Type::ArrayStart ||
          context.token.value_type == Type::ObjectStart)
        level++;
      else if (context.token.value_type == Type::ArrayEnd ||
               context.token.value_type == Type::ObjectEnd)
        level--;
    }
    if (buffer_change)
  ...
  static inline void from(const std::vector<Token> &from_type, Token &token,
                          Serializer &serializer) {
...
public:
  ...
  static inline void from(const JsonTokens &from, Token &token,
                          Serializer &serializer) {
...
template <> struct TypeHandler<JsonArrayRef> {
  static inline Error to(JsonArrayRef &to_type, ParseContext &context) {
    ...
    to_type.ref.size = size_t(context.token.value.data +
                              context.token.value.size - to_type.ref.data);

    return error;
  }

  static inline void from(const JsonArrayRef &from_type, Token &token,
                          Serializer &serializer) {
...
template <> struct TypeHandler<JsonArray> {
  ...
  static inline void from(const JsonArray &from_type, Token &token,
                          Serializer &serializer) {
    token.value_type = JS::Type::Verbatim; // Need to fool the serializer to
                                           // just write value as verbatim
...
template <> struct TypeHandler<JsonObjectRef> {
  ...
  static inline void from(const JsonObjectRef &from_type, Token &token,
                          Serializer &serializer) {
    token.value = from_type.ref;
    token.value_type = Type::Verbatim;
    serializer.write(token);
...
template <> struct TypeHandler<JsonObject> {
  ...
  static inline void from(const JsonObject &from_type, Token &token,
                          Serializer &serializer) {
    token.value_type = JS::Type::Verbatim; // Need to fool the serializer to
                                           // just write value as verbatim
...
template <> struct TypeHandler<JsonObjectOrArrayRef> {
  ...
  static inline void from(const JsonObjectOrArrayRef &from_type, Token &token,
                          Serializer &serializer) {
    token.value = from_type.ref;
    token.value_type = Type::Verbatim;
    serializer.write(token);
...
template <> struct TypeHandler<JsonObjectOrArray> {
  ...
  static inline void from(const JsonObjectOrArray &from_type, Token &token,
                          Serializer &serializer) {
    token.value_type = JS::Type::Verbatim; // Need to fool the serializer to
                                           // just write value as verbatim
...
template <size_t INDEX, typename... Ts> struct TupleTypeHandler {
  static inline Error to(JS::Tuple<Ts...> &to_type, ParseContext &context) {
    ...
    Error error = TypeHandler<Type>::to(
        to_type.template get<sizeof...(Ts) - INDEX>(), context);
    if (error != JS::Error::NoError)
      return error;
    error = context.nextToken();
    if (error != JS::Error::NoError)
  ...
  static inline void from(const JS::Tuple<Ts...> &from_type, Token &token,
                          Serializer &serializer) {
    using Type = typename JS::TypeAt<sizeof...(Ts) - INDEX, Ts...>::type;
    TypeHandler<Type>::from(from_type.template get<sizeof...(Ts) - INDEX>(),
                            token, serializer);
...
template <typename... Ts> struct TypeHandler<JS::Tuple<Ts...>> {
  static inline Error to(JS::Tuple<Ts...> &to_type, ParseContext &context) {
    ...
    if (error != JS::Error::NoError)
      return error;
    error = JS::Internal::TupleTypeHandler<sizeof...(Ts), Ts...>::to(to_type,
                                                                     context);
    if (error != JS::Error::NoError)
      return error;
  ...
  static inline void from(const JS::Tuple<Ts...> &from_type, Token &token,
    ...
    JS::Internal::TupleTypeHandler<sizeof...(Ts), Ts...>::from(from_type, token,
                                                               serializer);
    token.name = DataRef("");
...
template <size_t INDEX, typename... Ts> struct StdTupleTypeHandler {
  static inline Error to(std::tuple<Ts...> &to_type, ParseContext &context) {
    using Type = typename std::tuple_element<sizeof...(Ts) - INDEX,
                                             std::tuple<Ts...>>::type;
    Error error = TypeHandler<Type>::to(
        std::get<sizeof...(Ts) - INDEX>(to_type), context);
    if (error != JS::Error::NoError)
      return error;
    error = context.nextToken();
    if (error != JS::Error::NoError)
  ...
  static inline void from(const JS::Tuple<Ts...> &from_type, Token &token,
    ...
    using Type = typename std::tuple_element<sizeof...(Ts) - INDEX,
                                             std::tuple<Ts...>>::type;
    TypeHandler<Type>::from(std::get<sizeof...(Ts) - INDEX>(from_type), token,
                            serializer);
...
template <typename... Ts> struct StdTupleTypeHandler<0, Ts...> {
  ...
  static inline void from(const std::tuple<Ts...> &from_type, Token &token,
                          Serializer &serializer) {
    JS_UNUSED(from_type);
    JS_UNUSED(token);
    JS_UNUSED(serializer);
...
template <typename... Ts> struct TypeHandler<std::tuple<Ts...>> {
  static inline Error to(std::tuple<Ts...> &to_type, ParseContext &context) {
    ...
    if (error != JS::Error::NoError)
      return error;
    error = JS::Internal::StdTupleTypeHandler<sizeof...(Ts), Ts...>::to(
        to_type, context);
    if (error != JS::Error::NoError)
      return error;
    if (context.token.value_type != JS::Type::ArrayEnd)
  ...
  static inline void from(const std::tuple<Ts...> &from_type, Token &token,
    ...
    JS::Internal::StdTupleTypeHandler<sizeof...(Ts), Ts...>::from(
        from_type, token, serializer);
    token.name = DataRef("");
...
template <typename T> struct OneOrMany { std::vector<T> data; };
...
public:
  ...
  static void from(const OneOrMany<T> &from, Token &token,
                   Serializer &serializer) {
    if (from.data.empty())
      return;
    if (from.data.size() > 1) {
...
struct TypeHandler<std::unordered_map<Key, Value>>
    : TypeHandlerMap<Key, Value, std::unordered_map<Key, Value>> {};
...
struct Map {
  struct It {
    ...
    It(const It &other)
        : map(other.map), index(other.index), next_meta(other.next_meta),
  ...
  inline It begin() const {
    ...
    b.index = 1;
    b.next_meta = 1;
    b.next_complex = b.next_meta < uint32_t(meta.size())
                         ? uint32_t(meta[b.next_meta].position)
                         : uint32_t(tokens.data.size());
    return b;
  }
  ...
  template <typename T>
  JS::Error castToType(const It &iterator, JS::ParseContext &parseContext,
                       T &to) const {
  ...
  template <typename T>
  JS::Error castToType(const std::string &name, JS::ParseContext &parseContext,
                       T &to) const {
    if (tokens.data.empty() ||
        tokens.data.front().value_type != JS::Type::ObjectStart) {
  ...
  JS::Error setValue(JS::ParseContext &parseContext, const T &value) {
    static_assert(
        sizeof(JS::Internal::HasJsonStructBase<T>::template test_in_base<T>(
            nullptr)) ==
    ...
    if (parseContext.error == JS::Error::NoError)
      assert(tokens.data.size() &&
             tokens.data[0].value_type == JS::Type::ObjectStart);
  ...
  template <typename T>
  JS::Error setValue(const std::string &name, JS::ParseContext &parseContext,
                     const T &value) {
    (void)parseContext;
    if (tokens.data.empty()) {
    ...
    serializeContext.serializer.setOptions(
        SerializerOptions(JS::SerializerOptions::Compact));
    JS::Token token;
    token.value_type = Type::ObjectStart;
    token.value = DataRef(objectStart);
    ...
    tokens.data.insert(tokens.data.end() - 1, new_tokens.data.begin() + 1,
                       new_tokens.data.end() - 1);
    meta[0].children++;
    if (new_meta[0].complex_children) {
      ...
      for (int new_meta_i = old_meta_size; new_meta_i < int(meta.size());
           new_meta_i++) {
        meta[new_meta_i].position +=
            old_tokens_size - 1 - 1; // position contains an extra and
                                     // old_tokens_size has another extra
      }
    } else {
      meta[0].size++;
    }
...
template <> struct TypeHandler<Map> {
  ...
  static inline void from(const Map &from_type, Token &,
                          Serializer &serializer) {
...
template <typename T, size_t COUNT> struct ArrayVariableContent {
  T data[COUNT];
  size_t size = 0;
};
...
struct TypeHandler<ArrayVariableContent<T, COUNT>> {
  static inline Error to(ArrayVariableContent<T, COUNT> &to_type,
                         ParseContext &context) {
    if (context.token.value_type != Type::ArrayStart)
      return JS::Error::ExpectedArrayStart;
  ...
  static inline void from(const ArrayVariableContent<T, COUNT> &from_type,
                          Token &token, Serializer &serializer) {
    token.value_type = Type::ArrayStart;
    token.value = DataRef("[");
    serializer.write(token);
...
template <typename T, typename Set> struct TypeHandlerSet {
  ...
  static inline void from(const Set &set, Token &token,
                          Serializer &serializer) {
    token.value_type = Type::ArrayStart;
    token.value = DataRef("[");
    serializer.write(token);
...
namespace JS {
template <typename Key, typename Value>
struct TypeHandler<std::map<Key, Value>>
    : TypeHandlerMap<Key, Value, std::map<Key, Value>> {};
} // namespace JS
#endif
...
namespace JS {
template <typename Key>
struct TypeHandler<std::unordered_set<Key>>
    : TypeHandlerSet<Key, std::unordered_set<Key>> {};
} // namespace JS
#endif
...
public:
  ...
  static void from(const std::array<T, N> &from, Token &token,
                   Serializer &serializer) {
    token.value_type = Type::ArrayStart;
    token.value = DataRef("[");
    serializer.write(token);
